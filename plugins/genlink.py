...
async def gen_link_batch(bot, message):
    if " " not in message.text:
        return await message.reply("ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ғᴏʀᴍᴀᴛ.\nᴇxᴀᴍᴘʟᴇ <code>/batch https://t.me/Novus_Updates/1 https://t.me/Novus_Updates/1</code>.")
    ...
    post = await bot.send_document(LOG_CHANNEL, f"batchmode_{message.from_user.id}.json", file_name="Batch.json", caption="⚠️ ɢᴇɴᴇʀᴀᴛᴇᴅ ғᴏʀ ғɪʟᴇsᴛᴏʀᴇ.")
    os.remove(f"batchmode_{message.from_user.id}.json")
    file_id, ref = unpack_new_file_id(post.document.file_id)
    await sts.edit(f"ʜᴇʀᴇ ɪs ʏᴏᴜʀ ʟɪɴᴋ\nᴄᴏɴᴛᴀɪɴs `{og_msg}` files.\n https://t.me/{temp.U_NAME}?start=BATCH-{file_id}")
